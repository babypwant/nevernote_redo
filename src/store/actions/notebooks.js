...
export const createNotebookThunk = (userid, title) => async (dispatch) => {
    let newNotebook = await fetch(`https://nevernote.herokuapp.com/api/users/${userid}/notebooks/`, {
        method: 'POST',
...
export const renameNotebookThunk = (userid, notebookid, title) => async (dispatch) => {
    let renamed = await fetch(
      `https://nevernote.herokuapp.com/api/users/${userid}/notebooks/${notebookid}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title }),
