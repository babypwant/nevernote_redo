...
import { addNoteToTag, deleteNoteFromTags } from './tags';
...
export const createNote = (userId, notebookId) => async (dispatch) => {
    const response = await fetch(
      `https://nevernote.herokuapp.com/api/users/${userId}/notebooks/${notebookId}/notes/`,
      {
        method: "POST",
        headers: {
...
export const updateNote = (userId, notebookId, noteId, content, title) => async (dispatch) => {
    const response = await fetch(
      `https://nevernote.herokuapp.com/api/users/${userId}/notebooks/${notebookId}/notes/${noteId}/`,
      {
        method: "PUT",
        headers: {
          "content-type": "application/json",
...
export const addTagToNoteThunk = (noteid, tagid) => async dispatch => {
    ...
    let newNoteTag = await fetch(
      `https://nevernote.herokuapp.com/api/notes/${noteid}/tags`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
