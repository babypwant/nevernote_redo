...
export const createTagThunk = (userid, name) => async dispatch => {
    ...
    let newTag = await fetch(
      `https://nevernote.herokuapp.com/api/users/${userid}/tags/`,
      {
        method: "POST",
        headers: {
...
export const deleteTagThunk = (userid, tagid) => async dispatch => {
    ...
    const res = await fetch(
      `https://nevernote.herokuapp.com/api/users/${userid}/tags/${tagid}`,
      {
        method: "DELETE",
      }
    );
...
export const disassociateTagThunk = (noteid, tagid) => async dispatch => {
  ...
  let response = await fetch(
    `https://nevernote.herokuapp.com/api/notes/${noteid}/tags/${tagid}`,
    {
      method: "DELETE",
    }
  );
